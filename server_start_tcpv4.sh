#!/bin/bash
# Script para iniciar el servidor TCP V4 en segundo plano
# Uso: ./server_start_tcpv4.sh
...
echo -e "${BLUE}🚀 INICIANDO SERVIDOR TCP V4${NC}"
echo "=================================="
...
if [ -f "$PID_FILE" ]; then
    ...
    if ps -p "$PID" > /dev/null 2>&1; then
        ...
        echo "   Para detenerlo: ./server_stop_tcpv4.sh"
        echo "   Para ver estado: ./server_status_tcpv4.sh"
        exit 1
    else
...
if ps -p "$SERVER_PID" > /dev/null 2>&1; then
    ...
    echo -e "   ${YELLOW}Ver estado:${NC} ./server_status_tcpv4.sh"
    echo -e "   ${YELLOW}Detener:${NC} ./server_stop_tcpv4.sh"
    echo -e "   ${YELLOW}Ver logs:${NC} tail -f $LOG_FILE"
    echo ""
    echo -e "${BLUE}📡 Servidor escuchando en puerto 5000${NC}"
else
